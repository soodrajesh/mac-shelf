...
/// The manifest MacShelf polls to learn a newer version exists. The source
/// file is still a small, hand-edited static JSON file alongside the
/// landing page (gogenops.com/mac-apps/macshelf/updates.json) —
/// `{"version": "1.1", "notes": "...", "url": "https://..."}` — but the poll
/// itself goes through `/api/update-check` instead of fetching that file
/// directly, so gogenops.com has a record of what versions are actually
/// still installed and running. The endpoint always serves the same
/// manifest either way; adding `app`/`v`/`os` query params changes nothing
...
/// MacShelf at an arbitrary binary) this app doesn't have yet. Silent
/// self-replacement is a deliberate follow-up, not something to bolt on
/// here.
enum UpdateCheckService {
    static func checkForUpdate() async -> UpdateManifest? {
        ...
        components.queryItems = [
            URLQueryItem(name: "app", value: "macshelf"),
