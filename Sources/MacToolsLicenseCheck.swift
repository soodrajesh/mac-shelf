...
/// taxonomy — pointed at a **separate** MacTools Pro product in Polar, per
...
/// MacTools Pro product/license-key benefit exists in the Polar dashboard:
...
///   from), or set the `MACTOOLS_POLAR_ORG_ID` environment variable to
///   override it without a rebuild (e.g. for a TestFlight-style dry run).
enum PolarConfig {
    // TODO(polar-setup): Create the "MacTools Pro" product + license-key
    ...
    static var checkoutURL: URL? {
        URL(string: ProcessInfo.processInfo.environment["MACTOOLS_POLAR_CHECKOUT_URL"]
            ?? "https://gogenops.com/mac-apps/mactools/")
    }
}
...
private enum SecureLicenseCache {
    private static let licenseService = "com.rajeshsood.mactools.license.cache.v1"

    /// Byte-masked so the secret isn't plain text in the binary's strings
    /// table. This is MacTools' own key, distinct from MacGroom's —
...
public enum LicenseCheckError: LocalizedError {
    ...
    public var errorDescription: String? {
        ...
        case .unknown(let message):
            return message
        case .wrongProduct:
            return "This license key isn't valid for MacTools."
        }
    }
}
...
/// the MacTools Pro benefit — kept on the struct for source parity with
